...
export const register = async (req, res) => {
    try {
        ...
        return res.status(201).json({
            success: true, message: 'Account Created Successully'
        })
    } catch (error) {
        console.log(error);  
...
export const followOrunfollow = async (req, res) => {
    try {
        const followkrneWala = req.id; // nrml id
        const jiskofollowkarunga = req.params.id; //shivani id
        ...
        //now main chck karunga follow karunkga ya unfollow
        const isFollowing = user.following.includes(jiskofollowkarunga)
        if (isFollowing) {
            //unfollow logic
            await Promise.all([
                User.updateOne({ _id: followkrneWala }, { $pull: { following: jiskofollowkarunga } }),
                User.updateOne({ _id: jiskofollowkarunga }, { $pull: { followers: jiskofollowkarunga } }),
            ])
            return res.status(200).json({
            ...
            await Promise.all([
                ...
                User.updateOne({ _id: jiskofollowkarunga }, { $push: { followers: jiskofollowkarunga } }),
            ])
            return res.status(200).json({
